...
describe('InfoDetailView', () => {
  ...
  it('keeps the reading hero focused and hides acquisition diagnostics from users', async () => {
    const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
      const url = String(input)
    ...
    expect(wrapper.find('[data-testid="info-detail-quickfacts"]').exists()).toBe(false)
    ...
    expect(heroText).not.toContain('评分 88')
    expect(heroText).not.toContain('complete')
    expect(wrapper.get('[data-testid="info-detail-meta-line"]').text()).toContain('热点事件 · 微博 · 2026-04-20 10:00:00')
    expect(wrapper.text()).not.toContain('采集信息')
