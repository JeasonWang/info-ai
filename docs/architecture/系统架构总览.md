...
当前数据库完整结构以 [../../info_aggregation/sql/mysql_schema_pro.sql](../../info_aggregation/sql/mysql_schema_pro.sql) 为准，初始化数据以 [../../info_aggregation/sql/mysql_migration_max.sql](../../info_aggregation/sql/mysql_migration_max.sql) 为准。
...
| MySQL 不由 compose 管理 | 首次部署依赖人工初始化 | 部署手册固定初始化和迁移流程 |
| GitHub Actions 只上传基础 SQL | 旧库升级需人工准备迁移脚本 | 生产升级前手动同步 `info_aggregation/sql/migration_*.sql` |
