...
func TestUserFavoriteEventsRouteUseBearerSession(t *testing.T) {
	r := transporthttp.NewRouter(transporthttp.Services{})
	registerPayload := bytes.NewBufferString(`{"email":"favorite-list@example.com","password":"StrongerPass123"}`)
	registerReq := httptest.NewRequest(http.MethodPost, "/api/v1/auth/register", registerPayload)
	registerReq.Header.Set("Content-Type", "application/json")
	registerRes := httptest.NewRecorder()
	r.ServeHTTP(registerRes, registerReq)
	...
	loginPayload := bytes.NewBufferString(`{"email":"favorite-list@example.com","password":"StrongerPass123"}`)
	loginReq := httptest.NewRequest(http.MethodPost, "/api/v1/auth/login", loginPayload)
	loginReq.Header.Set("Content-Type", "application/json")
	loginRes := httptest.NewRecorder()
	r.ServeHTTP(loginRes, loginReq)
	...
	listReq := httptest.NewRequest(http.MethodGet, "/api/v1/me/favorite-events", nil)
	listReq.Header.Set("Authorization", "Bearer "+loginBody.Data.Token)
	listRes := httptest.NewRecorder()
	r.ServeHTTP(listRes, listReq)
	if listRes.Code != http.StatusOK {
		t.Fatalf("list favorite events status = %d, body=%s", listRes.Code, listRes.Body.String())
	}
	...
	if err := json.Unmarshal(listRes.Body.Bytes(), &listBody); err != nil {
		t.Fatalf("invalid favorite events json: %v", err)
	}
	if len(listBody.Data) != 1 || listBody.Data[0].ID != 101 || listBody.Data[0].TargetPath != "/events/101" {
		t.Fatalf("favorite events = %+v", listBody.Data)
	}
}
