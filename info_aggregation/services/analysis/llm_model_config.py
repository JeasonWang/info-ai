...
def select_available_llm_config(session) -> LLMModelConfig | None:
    ...
    for config in configs:
        ...
        if limit > 0 and (config.daily_call_count or 0) >= limit:
            continue
        return config
    return None
...
def record_llm_call_success(
    ...
    config.circuit_open_until = None
    config.last_failure_reason = ""
    log = _record_llm_call_log(session, config, "succeeded", latency_ms, input_item_count)
    session.flush()
    return log
...
def record_llm_call_failure(
    ...
    log = _record_llm_call_log(session, config, "failed", latency_ms, input_item_count, error_message)
    session.flush()
    return log
