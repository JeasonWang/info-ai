...
def analyze_event_sources(
    ...
    try:
        ...
        if selected_config is not None:
            try:
                record_llm_call_success_independent(selected_config.id, latency_ms, len(items))
        ...
        if session is not None and selected_config is not None:
            ...
            try:
                record_llm_call_failure_independent(selected_config.id, latency_ms, len(items), str(exc))
            except Exception as log_exc:
