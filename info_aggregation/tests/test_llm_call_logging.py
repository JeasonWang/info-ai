...
def test_event_analysis_returns_llm_result_when_success_log_write_fails(session, monkeypatch):
    ...
    def fail_success_log(*args, **kwargs):
        raise OperationalError("UPDATE llm_model_config", {}, RuntimeError("lock wait timeout"))

    monkeypatch.setattr("services.event_analysis.pipeline.build_llm_provider_from_config", lambda selected: FakeProvider())
    monkeypatch.setattr("services.event_analysis.pipeline.record_llm_call_success_independent", fail_success_log)
    ...
    assert config.daily_call_count == 0
